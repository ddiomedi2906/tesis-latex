...
resource. The ICF weight is proposed given that the \textit{Inverse Document Frequency} (IDF) 
...
for a given surface form. The intuition behind the ICF formula is that the discriminative power of a word is inversely 
...
A web-service\footnote{https://www.dbpedia-spotlight.org/} is available for integration with 
...
Tagger\footnote{https://nlp.stanford.edu/software/CRF-NER.shtml} based on a Conditional Random 
...
incoming Wikipedia links that both entities share in their Wikipedia articles, which are referred
using crossreferencing properties such as \textit{same-as}. This approach is
polished up by considering the total number of entities in the Wikipedia collection~\cite{infExtr:MilneW08}.
...
AIDA provides a HTTP JSON web service\footnote{https://www.mpi-inf.mpg.de/departments/databases-and-information-systems/research/ambiverse-nlu/aida} 
for annotating texts. It can be accessed via CURL requests and only needs to be provided by the 
text that wants to be annotated.
...
the number of times the phrase $a$ occurs as an anchor in Wikipedia, and the frequency this 
phrase occurs in both anchor and non-anchor occurrences.

The coherence between two candidate annotations is equivalent to the process followed in the AIDA 
system. A score $\rho(a \rightarrow p)$ is then calculated per candidate annotation, which is 
compared to a threshold $\rho_{NA}$, so annotations with a score lower than the given threshold 
are pruned by setting $a \rightarrow NA$ (a fake page to denote pruned annotations). Two 
approaches that combine $lp$ and coherence are presented to compute this score: one is an 
...
unigram language model trained from Wikidata items labels.
...
Therefore, the \textbf{semantic similarity} measure is used to make the process 
...
and defines the probability that two random walks starting from $e$ and $e'$ end up on the 
same item.
...
independently along the Markov chain. This allows to record the features at each step, 
...
OpenTapioca is available through a web-service\footnote{https://opentapioca.org/} implemented 
using Solr\footnote{https://lucene.apache.org/solr/} and some Python libraries. Its service keeps 
...
where wiGloVe is a precomputed GloVe embedding~\cite{seqlab:PenningtonSM14}. Note that these 
word representations are the input words for the BiLSTM-CRF. An example is shown in 
